...
@time begin
    for ens in ensInfo
        ...
        for impr_set in IMPR_SET
            ...
            for discr in ["ll","lc"]
                HVP["gSS_$discr"] = 3*HVP["g88conn_$discr"] - HVP["g33_$discr"]
                if wind in ["NW","ILD","LD","LD2"]
                    info["HVPsyst"]["gSS_$discr"] = sqrt(9*info["HVPsyst"]["g88conn_$discr"]^2 + info["HVPsyst"]["g33_$discr"]^2)
                end
            end
...
@time begin
    for ens in ensInfo
        ...
        if WRITE
            ...
            RESstr = RESC ? "_resc" : ""
            BLstr  = BLIND ? "Blind" : ""
            pBDIO = create_path(path_bdio,["HVP&FVC",wind,ens.id,"$(ens.id)_$(BLstr)FVC$(diag)$(RESstr)$(REFstr)"],OVERWRITE=true)
