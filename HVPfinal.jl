...
for diag in ["NLOa","NLOb","NLOa&b"]
    ...
    uwerr(amu[diag]["SDsub"]["CC"]); der_mDs = mchist(amu[diag]["SDsub"]["CC"], "MD_ph [GeV]")[1] / artificial_err
    amu[diag]["SDsub"]["CC"]  += value(MD_ph - MD_ph_prime) * der_mDs
    uwerr(amu[diag]["SDsub"]["∆lc"]); der_mDs = mchist(amu[diag]["SDsub"]["∆lc"], "MD_ph [GeV]")[1] / artificial_err
    amu[diag]["SDsub"]["∆lc"] += value(MD_ph - MD_ph_prime) * der_mDs
    uwerr(amu[diag]["SD"]["CC"]); der_mDs = mchist(amu[diag]["SD"]["CC"], "MD_ph [GeV]")[1] / artificial_err
...
for diag in ["NLOa","NLOb","NLOa&b"]
    for wind in ["SD","ID","LD"]
        ...
        if wind == "SD"
            println("   (aµ[$diag])($wind) = $(print_uwreal(10*(AMU[diag][wind] - (1/9)*amu_bb[diag]),10*[AMUSYST[diag][wind],AMUFVC[diag][wind]],total=true))")
        else
            println("   (aµ[$diag])($wind) = $(print_uwreal(10*AMU[diag][wind],10*[AMUSYST[diag][wind],AMUFVC[diag][wind]],total=true))")
        end
    end
    println("--------------------------------------------------------")
end
...
RES_WP = Dict(
    "NLOa" => Dict(
        "part" => [
            ...
            [uwreal([-20.755 ,0.075],"KNT19 NLO"),"KNT19"],            
            # [uwreal([-21.34 ,0.13 ],"KNT19/CMD3 NLO"),"KNT19/CMD-3"]
            [uwreal([-21.338,0.126],"KNT19/CMD3 NLO"),"KNT19/CMD-3"]
        ],
        # "aver" => [uwreal([-21.045,0.295],"WP25"),"WP25"]
        "aver" => [uwreal([-21.0465,0.292],"WP25"),"WP25"]
    ),
    "NLOb" => Dict(
        "part" => [
            # [uwreal([10.59 ,0.04 ],"KNT19 NLO"),"KNT19"],
            [uwreal([10.589,0.037],"KNT19 NLO"),"KNT19"],
            # [uwreal([10.92 ,0.07 ],"KNT19/CMD3 NLO"),"KNT19/CMD-3"]
            [uwreal([10.920,0.068],"KNT19/CMD3 NLO"),"KNT19/CMD-3"]
        ],
    ...
    "NLOc" => Dict(
        "part" => [
            ...
            [uwreal([0.340,0.010],"KNT19 NLO"),"KNT19"],
            # [uwreal([0.36 ,0.01 ],"KNT19/CMD3 NLO"),"KNT19/CMD-3"]
            [uwreal([0.361,0.011],"KNT19/CMD3 NLO"),"KNT19/CMD-3"]
        ],
        # "aver" => [uwreal([0.35,0.01],"WP25"),"WP25"]
        "aver" => [uwreal([0.351,0.011],"WP25"),"WP25"]
    ),
    "NLO" => Dict(
        "part" => [
...
\\begin{table}[t]
    ...
    S_mK     = (mchist(AMU[diag][comp], "mK_ph")[1] / mK_ph.err) * (mK_ph.mean/AMU[diag][comp].mean)
    S_mDs    = (mchist(AMU[diag][comp], "MD_ph [GeV]")[1] / artificial_err) * (MD_ph.mean/AMU[diag][comp].mean)
println("        \$a_\\mu^{\\mathrm{hvp}}\$ & $(round(S_sqrtt0,digits=2)) & $(round(S_mPi,digits=2)) & $(round(S_mK,digits=2)) & $(round(S_mDs,digits=2)) \\\\")
println("    \\end{tabular}
\\end{table}
")
