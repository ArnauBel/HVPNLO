...
if !VREF33
    ...
    println(" => amu($diag;SD;33) = $(round(value(amu33SD)*factor,digits=digits))($(round(err(amu33SD)*factor,digits=digits)))($(round(amu33SDsyst*factor,digits=digits)))($(round(amu33SDt0err*factor,digits=digits)))($(round(abs(amuSDfvc)*factor,digits=digits)))[$(round(amu33SDerr*factor,digits=digits))]")
end
...
if !VREF33
    ...
    println(" => amu($diag;SD) = $(round(value(amuSD)*factor,digits=digits))($(round(err(amuSD)*factor,digits=digits)))($(round(amuSDsyst*factor,digits=digits)))($(round(amuSDt0err*factor,digits=digits)))($(round(abs(amuSDfvc)*factor,digits=digits)))[$(round(amuSDerr*factor,digits=digits))]")
end
...
if !VREF33
    ...
    println(" => amu($diag;$wind;33) = $(round(value(amu33)*factor,digits=digits))($(round(err(amu33)*factor,digits=digits)))($(round(amu33syst*factor,digits=digits)))($(round(amu33t0err*factor,digits=digits)))($(round(abs(amufvc)*factor,digits=digits)))[$(round(amu33err*factor,digits=digits))]")
end
...
if !VREF33
    ...
    println(" => amu($diag;$wind) = $(round(value(amu)*factor,digits=digits))($(round(err(amu)*factor,digits=digits)))($(round(amusyst*factor,digits=digits)))($(round(amut0err*factor,digits=digits)))($(round(abs(amufvc)*factor,digits=digits)))[$(round(amuerr*factor,digits=digits))]")
end
...
amuLDerr   = sqrt(err(amuLD)^2 + amuLDsyst^2 + amuLDt0err^2) : sqrt(err(amuLD)^2 + amuLDsyst^2 + amuLDt0err^2 + amuLDfvc^2)
...
if !VREF
    ...
    println(" => $(charge_factor[comp*"s"]) amu($diag;$comp) = $(round(value(amu)*factor,digits=digits))($(round(err(amu)*factor,digits=digits)))($(round(amusyst*factor,digits=digits)))($(round(amut0err*factor,digits=digits)))[$(round(amuerr*factor,digits=digits))]")
        ...
        println(" Window decomposition:")
    println("---------------------------------------------------------")
    println(" => $(charge_factor[comp*"s"]) amu($diag;SD;$comp) = $(round(value(amuSD)*factor,digits=digits))($(round(err(amuSD)*factor,digits=digits)))($(round(amuSDsyst*factor,digits=digits)))($(round(amuSDt0err*factor,digits=digits)))($(round(amuSDfvc*factor,digits=digits)))[$(round(amuSDerr*factor,digits=digits))]")
    println(" => $(charge_factor[comp*"s"]) amu($diag;ID;$comp) = $(round(value(amuID)*factor,digits=digits))($(round(err(amuID)*factor,digits=digits)))($(round(amuIDsyst*factor,digits=digits)))($(round(amuIDt0err*factor,digits=digits)))($(round(amuIDfvc*factor,digits=digits)))[$(round(amuIDerr*factor,digits=digits))]")
    println(" => $(charge_factor[comp*"s"]) amu($diag;LD;$comp) = $(round(value(amuLD)*factor,digits=digits))($(round(err(amuLD)*factor,digits=digits)))($(round(amuLDsyst*factor,digits=digits)))($(round(amuLDt0err*factor,digits=digits)))($(round(amuLDfvc*factor,digits=digits)))[$(round(amuLDerr*factor,digits=digits))]")


    println(" Final estimation for diag $diag comp $comp")
    println("---------------------------------------------------------")
    println(" => $(charge_factor[comp*"s"]) amu($diag;$comp) = $(round(value(amu)*factor,digits=digits))($(round(err(amu)*factor,digits=digits)))($(round(amusyst*factor,digits=digits)))($(round(amut0err*factor,digits=digits)))($(round(amufvc*factor,digits=digits)))[$(round(amuerr*factor,digits=digits))]")
end
...
if !VREF
    ...
    println(" => amu($diag;LD) = $(round(value(amu["LD"])*factor/BLIND_LD_factor,digits=digits))($(round(err(amu["LD"])*factor/BLIND_LD_factor,digits=digits)))($(round(amusyst["LD"]*factor/BLIND_LD_factor,digits=digits)))($(round(amut0err["LD"]*factor/BLIND_LD_factor,digits=digits)))[$(round(amuerr["LD"]*factor/BLIND_LD_factor,digits=digits))]")


    println(" Final estimation for diag $diag")
    println("---------------------------------------------------------")
    println(" => amu($diag) = $(round(value(AMU)*factor,digits=digits))($(round(err(AMU)*factor,digits=digits)))($(round(AMUSYST*factor,digits=digits)))($(round(AMUT0ERR*factor,digits=digits)))[$(round(AMUERR*factor,digits=digits))]")
else
    println(" Window decomposition:")
    println("---------------------------------------------------------")
    println(" => amu($diag;SD) = $(round(value(amu["SD"])*factor,digits=digits))($(round(err(amu["SD"])*factor,digits=digits)))($(round(amusyst["SD"]*factor,digits=digits)))($(round(amut0err["SD"]*factor,digits=digits)))($(round(amufvc["SD"]*factor,digits=digits)))[$(round(amuerr["SD"]*factor,digits=digits))]")
    println(" => amu($diag;ID) = $(round(value(amu["ID"])*factor,digits=digits))($(round(err(amu["ID"])*factor,digits=digits)))($(round(amusyst["ID"]*factor,digits=digits)))($(round(amut0err["ID"]*factor,digits=digits)))($(round(amufvc["ID"]*factor,digits=digits)))[$(round(amuerr["ID"]*factor,digits=digits))]")
    println(" => amu($diag;LD) = $(round(value(amu["LD"])*factor/BLIND_LD_factor,digits=digits))($(round(err(amu["LD"])*factor/BLIND_LD_factor,digits=digits)))($(round(amusyst["LD"]*factor/BLIND_LD_factor,digits=digits)))($(round(amut0err["LD"]*factor/BLIND_LD_factor,digits=digits)))($(round(amufvc["LD"]*factor/BLIND_LD_factor,digits=digits)))[$(round(amuerr["LD"]*factor/BLIND_LD_factor,digits=digits))]")


    println(" Final estimation for diag $diag")
    println("---------------------------------------------------------")
    println(" => amu($diag) = $(round(value(AMU)*factor,digits=digits))($(round(err(AMU)*factor,digits=digits)))($(round(AMUSYST*factor,digits=digits)))($(round(AMUT0ERR*factor,digits=digits)))($(round(AMUFVC*factor,digits=digits)))[$(round(AMUERR*factor,digits=digits))]")
end
