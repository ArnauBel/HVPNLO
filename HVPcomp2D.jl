...
@time begin
    for ens in [ensInfo[2]]
        ...
        for impr_set in IMPR_SET
            ...
            if tcut0 * aens > tcut_fix
                error("Choose larger tcut0")
            end
            ...
            tcutmax = tcutmax == "max" ? t[end-1] : tcutmax
            ...
            for discr in ["ll","lc"] # ["ll-ll","ll-lc","lc-lc"] or ["ll","lc","mixed"]
                ...
                if ens.kappa_l == ens.kappa_s || ens.id ∈ ensNOdisc
                    ...
                    for tcut in ProgressBar(tcut0:tstep:tcutmax)  # ProgressBar(tcut0:tstep:t[end-1])
                    ...
                    for tcut in ProgressBar(tcut0:tstep:tcutmax)
