        ...
        for impr_set in IMPR_SET
            ...
            for discr in ["ll","lc"] # ["ll-ll","ll-lc","lc-lc"] or ["ll","lc","mixed"]
                ...
                if ens.kappa_l == ens.kappa_s || ens.id ∈ ensNOdisc
                    ...
                    if aens*tcut0 > tcut_fix
                        Eeff33=Eeff(Int(round(tcut_fix/aens.mean,RoundDown)), obs33)
                    end

                    for tcut in ProgressBar(tcut0:tstep:tcut_max)  # ProgressBar(tcut0:tstep:t[end-1])
                    ...
                    for tcut in ProgressBar(tcut0:tstep:tcut_max)
                ...
                if (ens.kappa_l == ens.kappa_s && ens.id ∉ ensNOdisc)
                    HVP["g8888_$(discr)$(discr)"] = HVP["g3388_$(discr)$(discr)"] = HVP["g3333_$(discr)$(discr)"]
                end
                ...
                if ens.id ∉ ensNOcharm
                    ...
                    for key in ((ens.kappa_l == ens.kappa_s || ens.id ∈ ensNOdisc) ? ["gCCCC_$(discr)$(discr)","g33CC_$(discr)$(discr)"] : ["gCCCC_$(discr)$(discr)","g33CC_$(discr)$(discr)","g88CC_$(discr)$(discr)"])
    ...
    for ens in ensInfo
        ...
        for impr_set in IMPR_SET
            ...
            for discr in ["ll","lc"]
                ...
                int_g3333 = TMRw .* (2*fvc33.*hcat(corr["g33_$discr"][t]...) .+ fvc33.*hcat(fvc33...))

                FVCg3333 = (alpha/pi)^3 * sum(int_g3333) * 1e10

                if ens.id ∉ ensNOdisc
                    if ens.kappa_l != ens.kappa_s
                        int_g8888 = TMRw .* (2*fvc88.*hcat(corr["g88_$discr"][t]...) .+ fvc88.*hcat(fvc88...))
                ...
                int_g3333_hp = TMRw .* (2*fvc33_hp.*hcat(corr["g33_$discr"][t]...) .+ fvc33_hp.*hcat(fvc33_hp...))
                ...
                if ens.id ∉ ensNOcharm
                    ...
                    for key in ((ens.kappa_l != ens.kappa_s && ens.id ∉ ensNOdisc) ? ["FVCg33CC_$(discr)$(discr)","FVCg88CC_$(discr)$(discr)","FVCg33CC_hp_$(discr)$(discr)"] : ["FVCg33CC_$(discr)$(discr)","FVCg33CC_hp_$(discr)$(discr)"])
...
# [uwerr(HVP[key]) for key in keys(HVP)]; HVP